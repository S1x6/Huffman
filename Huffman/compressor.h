...
Tree * makeTree(FILE * fin, int * table);						//формирует дерево Хаффмана на основе таблицы частот
...
char writeLeftBits(FILE * fout);								//дописыает 0 до конца байта, возвращает количество дописанных бит
void makeCodeTable(Tree * tree, char * code, char ** table);	//формирует таблицу кодов на основе дерева
void fillTable(char ** t);										//заполняет таблицу пустыми строками
char countBits(int * f, char ** c);								//считает количество бит на запись исходного файла
